# 资源管理_lite.py - 精简版
# 两分类：视频目录 + 文件浏览
# 视频目录=播放列表模式，文件浏览=单文件播放模式，固定自然排序升序，自动加载同名弹幕，文件夹封面
...
class Spider(Spider):
    ...
    def init(self, extend=""):
        ...
        self.FBROWSE_PREFIX = 'fbrowse://'
        self.FBFILE_PREFIX = 'fbfile://'
        self.dir_cache = {}
        self.dir_cache_time = {}
    ...
    def homeContent(self, filter_):
        classes = [
            {"type_id": "video_scan", "type_name": "📺 视频目录（列表）"},
            {"type_id": "file_browse", "type_name": "📁 文件浏览（单集）"},
        ]
    ...
    def categoryContent(self, tid, pg, filter_, extend):
        ...
        if tid == "file_browse":
            return self._file_browse_content(pg)

        # 解析当前路径
        current_path = None
        from_video_scan = False
        from_file_browse = False
        ...
        elif tid.startswith(self.FBROWSE_PREFIX):
            current_path = self.b64u_decode(tid[len(self.FBROWSE_PREFIX):])
            from_file_browse = True
        ...
        # 文件浏览模式：返回文件列表（单文件播放）
        if from_file_browse:
            return self._browse_directory(current_path, pg, from_video_scan=False, from_file_browse=True)

        return self._browse_directory(current_path, pg, from_video_scan)

    # ==================== 目录浏览 ====================

    def _browse_directory(self, current_path, pg, from_video_scan=False, from_file_browse=False):
        ...
        vlist = []
        parent_item = self._create_parent_item(current_path, from_video_scan, from_file_browse)
        if parent_item:
            vlist.append(parent_item)

        for f in page_files:
            item = self._create_file_item(f, from_video_scan, from_file_browse)
            if item:
                vlist.append(item)
    ...
    def _file_browse_content(self, pg):
        """文件浏览：列出所有视频目录，点进去是文件列表（单文件播放）"""
        ...
        for d in page_dirs:
            ...
            vlist.append({
                'vod_id': self.FBROWSE_PREFIX + self.b64u_encode(d['path']),
                'vod_name': f"📁 {d['name']}",
                'vod_pic': cover,
    ...
    def _create_parent_item(self, current_path, from_video_scan=False, from_file_browse=False):
        ...
        if os.path.normpath(parent) == os.path.normpath(self.root_path.rstrip('/')):
            if from_video_scan:
                ...
                parent_name = "视频目录"
            elif from_file_browse:
                parent_id = "file_browse"
                parent_name = "文件浏览"
            else:
                parent_id = "root_0"
                parent_name = "根目录"
            ...
            if from_video_scan:
                ...
                parent_id = self.FBROWSE_PREFIX + self.b64u_encode(parent)
            else:
    ...
    def _create_file_item(self, f, from_video_scan=False, from_file_browse=False):
        icon = self.get_file_icon(f['ext'], f['is_dir'])
        if f['is_dir']:
            if from_video_scan:
                prefix = self.VFOLDER_PREFIX
            elif from_file_browse:
                prefix = self.FBROWSE_PREFIX
            else:
        ...
        if self.is_media_file(f['ext']):
            if from_file_browse:
                media_id = self.FBFILE_PREFIX + self.b64u_encode(f['path'])
            else:
                media_id = f['path']
            return {
                'vod_id': media_id,
    ...
    def detailContent(self, ids):
        ...
        if id_val in ('video_scan', 'file_browse') or id_val.startswith('root_'):
            return self.categoryContent(id_val, 1, None, None)

        # 文件浏览前缀 → 进入目录浏览（单文件播放）
        if id_val.startswith(self.FBROWSE_PREFIX):
            folder_path = self.b64u_decode(id_val[len(self.FBROWSE_PREFIX):])
            if os.path.exists(folder_path) and os.path.isdir(folder_path):
                return self._browse_directory(folder_path, 1, from_video_scan=False, from_file_browse=True)
            return {'list': []}
        ...
        # 文件浏览的视频文件 → 单文件播放
        if id_val.startswith(self.FBFILE_PREFIX):
            file_path = self.b64u_decode(id_val[len(self.FBFILE_PREFIX):])
            if os.path.exists(file_path):
                return self._handle_file_detail(file_path, from_file_browse=True)
            return {'list': []}
    ...
    def _folder_play_list(self, folder_path, from_video_scan=False):
        ...
        cover = self._find_cover_image(folder_path) or self._get_folder_icon()

        play_urls = []
        for v in all_videos:
            ep_name = os.path.splitext(v['name'])[0]
            play_urls.append(f"{ep_name}$file://{v['path']}")
    ...
    def _handle_file_detail(self, file_path, from_file_browse=False):
        name = os.path.basename(file_path)
        ext = self.get_file_ext(name)
        play_name = os.path.splitext(name)[0]
        is_media = self.is_media_file(ext)
        pic = self._get_video_icon() if is_media else self._get_folder_icon()
        play_url = f"{play_name}$file://{file_path}"
        base_source = '文件浏览' if from_file_browse else '播放'

        vod = {
            'vod_id': file_path,
            'vod_name': f"🎬 {name}",
            'vod_pic': pic,
            'vod_play_from': base_source,
